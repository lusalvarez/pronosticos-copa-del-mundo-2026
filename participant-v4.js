  ...
  try {
    ...
    const firebaseData = {
      ...
      predictions: matches.map((match, index) => ({
        ...
        date: match.date,
        stage: match.stage,
        prediction: predictions[index] || { home: "", away: "", firstGoal: "", penaltyWinner: "" }
      }))
    };
...
function showConfirmationModal(dayGroup, dayPredictions, completedCount, totalCount, dayIndex) {
  // Validation : vérifier les erreurs de saisie des penalties
  let hasErrors = false;
  const errors = [];
  ...
  dayGroup.matches.forEach(match => {
    ...
    // Vérifier si c'est un match à élimination directe avec un vainqueur aux penalties mais un score non nul
    if (isPlayoffMatch(match) && pred && pred.penaltyWinner && pred.penaltyWinner !== "") {
      const homeScore = pred.home;
      const awayScore = pred.away;
      
      // Si les scores sont remplis et différents, c'est une erreur
      if (homeScore !== "" && awayScore !== "" && homeScore !== awayScore) {
        hasErrors = true;
        errors.push({
          match: `${match.homeTeam} vs ${match.awayTeam}`,
          score: `${homeScore}-${awayScore}`,
          penaltyWinner: pred.penaltyWinner === "home" ? match.homeTeam : match.awayTeam
        });
      }
    }
  });
  
  // Si des erreurs sont détectées, afficher un message et bloquer l'envoi
  if (hasErrors) {
    let errorMessage = "❌ ERROR: No puedes seleccionar un ganador en los penales si el pronóstico NO es un empate.\n\n";
    errorMessage += "Partidos con error:\n\n";
    errors.forEach(err => {
      errorMessage += `• ${err.match}\n`;
      errorMessage += `  Pronóstico: ${err.score} (no es empate)\n`;
      errorMessage += `  Ganador en penales: ${err.penaltyWinner}\n\n`;
    });
    errorMessage += "Por favor, corrige estos pronósticos antes de enviar.";
    alert(errorMessage);
    return;
  }
  ...
  // Vérifier si cette journée contient des matchs à élimination directe
  const hasPlayoffMatches = dayGroup.matches.some(match => isPlayoffMatch(match));
  
  let tableHTML = `
    <div style="background: #f3f4f6; padding: 0.75rem; font-weight: bold; border-bottom: 2px solid #d1d5db;">
      <div style="display: grid; grid-template-columns: ${hasPlayoffMatches ? '2fr 1fr 1fr 1fr' : '2fr 1fr 1.2fr'}; gap: 0.5rem; font-size: 0.9rem;">
        <div>Partido</div>
        <div style="text-align: center;">Pronóstico</div>
        <div style="text-align: center;">1er gol</div>
        ${hasPlayoffMatches ? '<div style="text-align: center;">Penales</div>' : ''}
      </div>
    </div>
  `;
  ...
  dayGroup.matches.forEach(match => {
    ...
    const penaltyWinnerText = isPlayoffMatch(match) && pred?.penaltyWinner
      ? (pred.penaltyWinner === "home" ? `🏆 ${match.homeTeam}` : `🏆 ${match.awayTeam}`)
      : (isPlayoffMatch(match) ? "➖ N/A" : "");
    
    tableHTML += `
      <div style="padding: 0.75rem; border-bottom: 1px solid #e5e7eb; ${isEmpty ? 'background: #fee2e2;' : 'background: #f0fdf4;'}">
        <div style="display: grid; grid-template-columns: ${hasPlayoffMatches ? '2fr 1fr 1fr 1fr' : '2fr 1fr 1.2fr'}; gap: 0.5rem; align-items: center; font-size: 0.9rem;">
          ...
          ${hasPlayoffMatches ? `<div style="text-align: center; font-weight: 700; color: ${pred?.penaltyWinner ? '#f59e0b' : '#6b7280'}; background: ${pred?.penaltyWinner ? '#fef3c7' : '#f3f4f6'}; padding: 0.35rem 0.5rem; border-radius: 6px;">${penaltyWinnerText}</div>` : ''}
        </div>
      </div>
    `;
  });
  ...
  try {
    ...
    Object.keys(validPredictions).forEach(index => {
      ...
      if (match) {
        ...
        updates[`participants/${participantId}/predictions/${index}/prediction/penaltyWinner`] = pred.penaltyWinner || "";
      ...
      if (snapshot.exists()) {
        ...
        if (savedPredictionsCount !== matches.length) {
          ...
          for (let i = savedPredictionsCount; i < matches.length; i++) {
            predictions[i] = { home: "", away: "", firstGoal: "", penaltyWinner: "" };
          }
        ...
        matches.forEach((match, index) => {
          if (!predictions[index]) {
            predictions[index] = { home: "", away: "", firstGoal: "", penaltyWinner: "" };
          }
        });
  ...
  dayGroups.forEach((dayGroup, dayIndex) => {
    ...
    dayGroup.matches.forEach((match) => {
      ...
      card.innerHTML = `
        ...
        <div class="penalty-winner-section" style="margin-top: 1rem; padding: 1rem; background: rgba(255, 193, 7, 0.1); border-radius: 8px;">
          <label style="display: flex; flex-direction: column; gap: 0.5rem;">
            <strong style="color: #f59e0b; font-size: 1rem;">🏆 ¿Quién gana en los penales? (solo si empate)</strong>
            <select
              data-index="${index}"
              data-side="penaltyWinner"
              class="penalty-winner-select"
              style="padding: 0.75rem; font-size: 1rem; border: 2px solid #f59e0b; border-radius: 8px; background: white; cursor: pointer;"
              ${isLocked ? 'disabled' : ''}
            >
              <option value="">-- Selecciona el ganador (opcional) --</option>
              <option value="home" ${prediction.penaltyWinner === "home" ? "selected" : ""}>${match.homeTeam}</option>
              <option value="away" ${prediction.penaltyWinner === "away" ? "selected" : ""}>${match.awayTeam}</option>
            </select>
          </label>
        </div>
        ` : ''}
